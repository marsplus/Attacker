...
public class Predict {
     ...
     */
    public static Vector<Double> doPredict(Vector<Double> aveFeaturePos , BufferedReader reader, BufferedReader JPG_reader , Writer writer, Model model) throws IOException {
        int correct = 0;
        int total = 0;
        double error = 0;
        double sump = 0, sumt = 0, sumpp = 0, sumtt = 0, sumpt = 0;
        double numP = 0, numN = 0, numTruePos = 0 , numTrueNeg = 0;
        ...
        while ((line = reader.readLine()) != null) {
            ...
            if (flag_predict_probability) {
                ...
                if( !(prob_estimates[0] > 0.99) )  
                	predict_label = 0;
                
                if( Math.abs(predict_label-1) < 1e-7 )
                {
                	printf(out , "%s" , imageId);
                	for(int k = 0; k < nodes.length; k++)
                		printf(out, " %d:%f", nodes[k].getIndex() , nodes[k].getValue());
                    printf(out, "\n");
                }
            ...
            if( target_label > 0 + 1e-7 )
            {
            	numP += 1;
            	if( predict_label > 0 + 1e-7)
            		numTruePos += 1;
            }
            else
            {
            	numN += 1;
            	if( Math.abs(predict_label - 0) < 1e-7 )
            		numTrueNeg += 1;
            }
        ...
        res.add( numTruePos / numP );
        res.add( numTrueNeg / numN );
        
        for(int i = 0; i < aveFeaturePos.size(); i++)
        	aveFeaturePos.set( i , aveFeaturePos.get(i)/sump );
    ...
    public static void main(String[] argv) throws IOException {
        ...
        try {
            ...
            doPredict(tmpVec , reader, reader , writer, model);
        }
        finally {
            closeQuietly(reader);
            closeQuietly(writer);
